...
class CamundaController extends Controller
{
    ...
    public function __invoke($id, $requestString = '/')
    {
        ...
        $req = $this->invokeRequest($id);

        return response()->json($req, 200);
    }

    protected function invokeRequest($instanceId)
    {
        foreach ($this->getCamundaClasses() as $class) {
            ...
            if ($this->hasIndetifier()) {
                $request->merge(['id' => $this->getIndetifier()]);
            }

            return $method->invoke($class, $request);
        }
    ...
    protected static function getNamespaceDirectory($namespace)
    {
